...
function EditableCell({ value, field, taskId, isEditing, onStartEdit, onFinish, type = 'text', options, className = '', displayValue, wrap = false }) {
  ...
  return (
    <div
      className={`w-full h-full flex items-center px-1 cursor-text ${wrap ? 'overflow-hidden' : 'truncate'} ${className}`}
      onDoubleClick={() => onStartEdit({ taskId, field })}
      title={field === 'name' && displayText.length > 20 ? displayText : undefined}
    >
      {wrap ? (
        <span className="line-clamp-2 break-words leading-tight text-[11px]">{displayText}</span>
      ) : (
        displayText
      )}
    </div>
