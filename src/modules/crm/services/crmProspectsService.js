...
async function searchLocalCrmData(filters = {}) {
  try {
    ...
    if (filters.search) {
      contactsQuery = contactsQuery.or(orIlike(['name', 'email', 'crm_companies.name'], filters.search));
    }
    ...
    if (filters.search) {
      companiesQuery = companiesQuery.or(orIlike(['name', 'cnpj', 'email'], filters.search));
    }
