/**
 * MetaSocialIntegration - Componente de conexao com Instagram + Facebook
...
export default function MetaSocialIntegration() {
  ...
  const handleConnect = async () => {
    ...
    try {
      await connectMeta();
      toast('Instagram e Facebook conectados com sucesso!', 'success');
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-slate-800 dark:text-slate-100">Instagram + Facebook</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                Publique videos e reels diretamente do sistema
              </p>
            </div>
          </div>
            ...
            <div className="space-y-3">
              <div className="bg-pink-50 dark:bg-pink-900/20 rounded-lg p-3">
                <p className="text-xs text-pink-700 dark:text-pink-300">
                  Conecte sua conta Instagram Business e pagina do Facebook para publicar videos,
                  reels e stories diretamente do sistema.
                </p>
              </div>

              <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-3">
                <p className="text-xs text-amber-700 dark:text-amber-300">
                  <strong>Requisitos:</strong> Conta Instagram Business ou Creator vinculada a uma
                  pagina do Facebook. Contas pessoais nao sao suportadas pela API.
                </p>
              </div>
              ...
              <button
                ...
                disabled={connecting}
                className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 hover:from-pink-600 hover:via-purple-600 hover:to-blue-600 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-all"
              >
                {connecting ? (
                ...
                {connecting ? 'Conectando...' : 'Conectar Instagram + Facebook'}
              </button>
            </div>
          ) : (
            /* Conectado */
      ...
      {showConfirmDisconnect && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowConfirmDisconnect(false)}>
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-sm w-full shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">Desconectar Instagram + Facebook?</h3>
